...
public class RecordedButton extends View {
    ...
    private Paint paintProgressBack;
    ...
    private void init() {
        ...
        dp11 = (int) getResources().getDimension(R.dimen.dimens_11);
        dp4 = (int) getResources().getDimension(R.dimen.dimens_4);
        ...
        paintProgress.setColor(colorWhite);
        paintProgress.setStrokeWidth(dp4);
        paintProgress.setStrokeCap(Paint.Cap.ROUND);
        paintProgress.setStyle(Paint.Style.STROKE);

        paintProgressBack = new Paint();
        paintProgressBack.setAntiAlias(true);
        paintProgressBack.setColor(Color.WHITE);
        paintProgressBack.setStrokeWidth(dp4);
        paintProgressBack.setStyle(Paint.Style.STROKE);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (Math.abs(moveX - firstX) > dp4 || Math.abs(moveY - firstY) > dp4) {
            ...
            case MotionEvent.ACTION_CANCEL:
                ...
                if (!cleanResponse){
                    if (isResponseLongTouch && !myHandler.hasMessages(0)) {
                        ...
                        myHandler.removeMessages(0);
                        if (Math.abs(upX - firstX) < dp4 && Math.abs(upY - firstY) < dp4) {
        ...
        if(buttonAnim == null || !buttonAnim.isRunning()) {
            ...
            buttonAnim.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                ...
                public void onAnimationUpdate(ValueAnimator animation) {
                    ...
                    radius2 = measuredWidth * (zoom - value) / 2 - dp11;

                    value = 1 - zoom - value;
                    oval.left = measuredWidth * value / 2 + dp11 / 2;
                    oval.top = measuredWidth * value / 2 + dp11 / 2;
                    oval.right = measuredWidth * (1 - value / 2) - dp11 / 2;
                    oval.bottom = measuredWidth * (1 - value / 2) - dp11 / 2;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if(measuredWidth == -1) {
            ...
            radius2 = measuredWidth* zoom /2 - dp11;

            oval.left = dp11 / 2;
            oval.top = dp11 / 2;
            oval.right = measuredWidth - dp11 / 2;
            oval.bottom = measuredWidth - dp11 / 2;
        }
    }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        shader1 = new LinearGradient(0, 0, measuredWidth, measuredWidth, getResources().getColor(R.color.startColor), getResources().getColor(R.color.endColor), Shader.TileMode.CLAMP);
        shader2 = new LinearGradient(measuredWidth / 2, 0, measuredWidth / 2, measuredWidth, getResources().getColor(R.color.endColor), getResources().getColor(R.color.startColor), Shader.TileMode.CLAMP);

        paint.setShader(shader1);
        paintProgress.setShader(shader2);
    }
...
//        canvas.drawCircle(measuredWidth/2, measuredWidth/2, radius1, paint);
        ...
        canvas.drawArc(oval, 270, 365, false, paintProgressBack);
        //绘制进度
